...
int	redire_checker(t_cmd **cmd, int type)
{
	...
	if ((*cmd)->type == type && (*cmd)->next->type == SPACE)
	{
		if (!(*cmd)->next->next)
			return (1);
		if ((*cmd)->next->next->type != WORD
			&& (*cmd)->next->next->type != VAR)
			return (1);
	}
	else if ((*cmd)->type == type && (*cmd)->next->type != WORD
		&& (*cmd)->next->type != VAR)
		return (1);
	return (0);
}
...
int	pipe_checker(t_cmd *cmd)
{
	...
	if (!cmd)
		return (0);
	if (pipe_checker_norm(p))
		return (1);
	if (!p.tmp->next)
		return (0);
	while (p.tmp->next->next)
		p.tmp = p.tmp->next;
	if (p.tmp->next->type == PIPE || (p.tmp->next->type == SPACE
			&& p.tmp->type == PIPE))
		return (1);
